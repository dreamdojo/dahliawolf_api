...
class Arcade
	{
		...
		public function getGameDetails($gameID, $increment = 1, $userID = "", $ip = "")
			{
				...
				if($getID['id'] > 0) {
					...
					if( ($userID != "") && ($userID != "null") ) 	{
						...
						$sql1 = "SELECT id FROM arcade_leaderboard WHERE user_id=\"".$userID."\" ORDER BY date_created DESC LIMIT 5";
						$query1 = mysql_query($sql1);
						while($result1 = mysql_fetch_array($query1))
							{
								$thumbData = getGameThumb($result1['id']);	
								$recentlyPlayed['id'] 		= $result1['id'];
								$recentlyPlayed['thumb'] 	= $thumbData;
							}
		...
		public function getGameThumb($gameID)
			{
				$db = new Database();
		...
		public function getPlayerPoints($userID, $gameID = 0, $today = 1)
			{
				//CHECK TO SEE IF THE GAME ID EXISTS
				$con = mysql_connect('localhost', 'dahlia_db', 'Jgv9EZ3G.WE6');
				mysql_select_db("dahlia_db", $con);
				...
				if($today == 1) {
					$sql1 = "SELECT SUM(points_earned) FROM arcade_leaderboard WHERE DATE(date_created) = DATE(NOW()) AND points_earned > 0 AND user_id=\"".$userID."\"";
					$query1 = mysql_query($sql1);
					$result1 = mysql_fetch_array($query1);
					
					$result['points_earned_today'] = $result1[0];
				}
				
				$sql = "SELECT user_id FROM `arcade_user_points` WHERE user_id=\"$userID\"";
				$getIDR = mysql_query($sql) or die(mysql_error());
				$getID 	= mysql_fetch_array($getIDR);
		...
		public function getPlayerRank($userID)
			{
				...
				$sql = "
					SELECT 
						(SELECT COUNT(*) FROM arcade_user_points pup WHERE (pup.total_points, pup.user_id) >= (aup.total_points, aup.user_id) ) AS rank
					FROM arcade_user_points aup
