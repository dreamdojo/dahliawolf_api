...
private $functions = array('NOW()');
	...
	public function prepareExec($sql, $values, $function = 'Custom') {
		 ...
		 if (!$this->sth->execute($values)) {
			if ($this->transaction) {
				$this->pdo->rollBack();
				$this->transaction = false;
			}
			$this->errorHandler($function, $sql);
	...
	private function errorHandler($function, $sql) {
		$errorInfo = $this->sth->errorInfo();
		$error = $_SERVER['REQUEST_URI'] . "\nMysql->" . $function . "() \n" . $errorInfo[2] . "\n" . $sql;
		if (DEVELOPMENT || !function_exists('logError')) { //development
			echo nl2br($error);
		}
		else { //production
			logError($error);
	...
	public function isForeignKey($field, &$foreignTable = NULL, &$foreignIndexes = NULL) {
		do {
			$foreignTable = substr($field, 0, strlen($field) - 3); // removes '_id'
