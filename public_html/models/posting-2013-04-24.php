...
class Posting extends db {
	...
	public function getPostDetails($params = array()) {
		...
		if (!empty($params['where']['posting_id'])) {
			...
			$from_prefix = '(SELECT MAX(posting_id) AS posting_id FROM posting) AS latest INNER JOIN posting ON latest.posting_id = posting.posting_id';
		}
	...
	public function allPosts($params = array()) {
		...
		if (!empty($params['like_day_threshold'])) {
			$select_str .= ', total_posting_likes.day_threshold_likes';
			$join_str .= '
				LEFT JOIN (
	...
	public function get_vote_posts($params = array()) {
		...
		$query = '
			SELECT posting.*, image.imagename, image.source
				, user_username.username, user_username.location, user_username.avatar
			...
			FROM posting
				...
				' . $join_str . '
				LEFT OUTER JOIN offline_commerce_v1_2013.product AS product ON posting_product.product_id = product.id_product
		...
		//AND (product.status = "Pending" OR product.status IS NULL)
		
		//LEFT OUTER JOIN vote_winner ON posting.posting_id = vote_winner.posting_id
		//AND vote_winner.posting_id IS NULL
	...
	public function get_num_post_likes_by_day($params = array()) {
		...
		$query = '
			SELECT posting.*, IFNULL(COUNT(posting_like.posting_like_id), 0) AS likes
			FROM posting
	...
	public function get_num_votes_by_period($params = array()) {
		...
		$query = '
			...
			FROM posting
				INNER JOIN posting_vote ON posting.posting_id = posting_vote.posting_id
			WHERE posting_vote.vote_period_id = :vote_period_id
				AND posting.posting_id = :posting_id
	...
	public function get_previous_posting_id($posting_id, $created, $viewer_user_id = NULL) {
		$query = '
			...
			FROM posting
				' . (!empty($viewer_user_id) ? 'LEFT JOIN posting_like ON posting.posting_id = posting_like.posting_id' : '') . '
			WHERE posting.created <= :created
				AND posting.posting_id != :posting_id
				AND posting.deleted IS NULL
				' . (!empty($viewer_user_id) ? 'AND (posting_like.user_id != :viewer_user_id OR posting_like.user_id IS NULL)' : '') . '
